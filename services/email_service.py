...
async def send_account_request_rejected(email: str, full_name: str, reason: str | None = None):
    reason_html = f"""
                    <div style="background-color: #0f3460; border-left: 4px solid #e94560; border-radius: 4px; padding: 20px; margin-top: 24px;">
                        <p style="color: #e94560; font-size: 13px; font-weight: bold; margin: 0 0 8px 0; letter-spacing: 1px;">MOTIVO</p>
                        <p style="color: #ffffff; font-size: 14px; margin: 0;">{reason}</p>
                    </div>
    """ if reason else ""

    message = MessageSchema(
        subject="Solicitud de cuenta - Sistema DAR (Dispositivo de Accion Rapida)",
        recipients=[email],
        body=f"""
        <!DOCTYPE html>
        <html>
        <body style="margin: 0; padding: 0; background-color: #f0f0f0; font-family: Arial, sans-serif;">
            <div style="max-width: 500px; margin: 30px auto; background-color: #1a1a2e; border-radius: 8px; overflow: hidden;">
                ...
                <div style="padding: 32px;">
                    <p style="color: #c0c0d0; font-size: 14px; line-height: 1.7; margin-top: 0;">
                        Hola {full_name}, tu solicitud de cuenta para el Sistema DAR fue revisada y no fue aprobada.
                    </p>
                    {reason_html}
                    <p style="color: #a0a0b0; font-size: 13px; margin-top: 24px;">
                        Si creés que se trata de un error, comunicate con el Ministerio de Seguridad.
                    </p>
                </div>
                <div style="background-color: #16213e; padding: 16px 32px; text-align: center; border-top: 1px solid #2a2a4a;">
                    <p style="color: #606070; font-size: 12px; margin: 0;">Ministerio de Seguridad de la Provincia de Jujuy</p>
...
async def send_password_reset_token(email: str, username: str, token: str):
    message = MessageSchema(
        subject="Recuperación de contraseña - Sistema DAR",
        recipients=[email],
        body=f"""
        <!DOCTYPE html>
        <html>
        <body style="margin: 0; padding: 0; background-color: #f0f0f0; font-family: Arial, sans-serif;">
            <div style="max-width: 500px; margin: 30px auto; background-color: #1a1a2e; border-radius: 8px; overflow: hidden;">
                ...
                <div style="padding: 32px;">
                    <p style="color: #c0c0d0; font-size: 14px; line-height: 1.7; margin-top: 0;">
                        Recibimos una solicitud para restablecer la contraseña del usuario <strong style="color:#fff;">{username}</strong>.
                    </p>
                    <div style="background-color: #0f3460; border-left: 4px solid #e94560; border-radius: 4px; padding: 20px; margin-top: 24px;">
                        <p style="color: #e94560; font-size: 13px; font-weight: bold; margin: 0 0 12px 0; letter-spacing: 1px;">TOKEN DE RECUPERACIÓN</p>
                        <p style="color: #ffffff; font-size: 22px; font-weight: bold; letter-spacing: 3px; margin: 0;">{token}</p>
                    </div>
                    <p style="color: #a0a0b0; font-size: 13px; margin-top: 24px;">⏱ Este token expira en <strong style="color:#fff;">15 minutos</strong>.</p>
                    <p style="color: #a0a0b0; font-size: 13px;">Si no solicitaste este cambio, ignorá este correo.</p>
                </div>
                <div style="background-color: #16213e; padding: 16px 32px; text-align: center; border-top: 1px solid #2a2a4a;">
                    <p style="color: #606070; font-size: 12px; margin: 0;">Ministerio de Seguridad de la Provincia de Jujuy</p>
                </div>
